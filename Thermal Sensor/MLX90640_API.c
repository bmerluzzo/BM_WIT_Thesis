...
{   
     return MLX90640_I2CRead(I2C1_DEV, slaveAddr, MLX90640_EEPROM_START_ADDRESS, MLX90640_EEPROM_DUMP_NUM, eeData);
}
...
int MLX90640_SynchFrame(uint8_t slaveAddr)
{
    ...
    error = MLX90640_I2CWrite(I2C1_DEV, slaveAddr, MLX90640_STATUS_REG, init_status_value);
    ...
    while(dataReady == 0)
    {
        error = MLX90640_I2CRead(I2C1_DEV, slaveAddr, MLX90640_STATUS_REG, 1, &statusRegister);
...
int MLX90640_TriggerMeasurement(uint8_t slaveAddr)
{
    ...
    error = MLX90640_I2CRead(I2C1_DEV, slaveAddr, MLX90640_CTRL_REG, 1, &ctrlReg);
    ...
    error = MLX90640_I2CWrite(I2C1_DEV, slaveAddr, MLX90640_CTRL_REG, cR);
...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    error = MLX90640_I2CRead(I2C1_DEV, slaveAddr, MLX90640_PIXEL_DATA_START_ADDRESS, MLX90640_PIXEL_NUM, frameData); 
    if(error != MLX90640_NO_ERROR)
    {
        return error;
    }                       
    
    error = MLX90640_I2CRead(I2C1_DEV, slaveAddr, MLX90640_AUX_DATA_START_ADDRESS, MLX90640_AUX_NUM, data); 
    if(error != MLX90640_NO_ERROR)
    {
        return error;
    }     
        
    error = MLX90640_I2CRead(I2C1_DEV, slaveAddr, MLX90640_CTRL_REG, 1, &controlRegister1);
...
int MLX90640_SetResolution(uint8_t slaveAddr, uint8_t resolution)
{
    ...
    if(error == MLX90640_NO_ERROR)
    {
        ...
        error = MLX90640_I2CWrite(I2C1_DEV, slaveAddr, MLX90640_CTRL_REG, v);        
    }    
...
//------------------------- To calculation -------------------------------------    
    ...
    for( int pixelNumber = 0; pixelNumber < 768; pixelNumber++)
    {
        ...
        {    
            ...
            Sx = (float) sqrt(sqrt(Sx)) * params->ksTo[1];            
            
            To = sqrt(sqrt(irData/(alphaCompensated * (1 - params->ksTo[1] * 273.15f) + Sx) + taTr)) - 273.15;                     
...
float MLX90640_GetTa(uint16_t *frameData, const paramsMLX90640 *params)
{
    ...
    ptatArt = (ptat / (ptat * params->alphaPTAT + (int16_t)frameData[768])) * (float)POW2(18);
...
static void ExtractPTATParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
    ...
    alphaPTAT = (eeData[16] & MLX90640_NIBBLE4_MASK) / (float)POW2(14) + 8.0f;
...
static void ExtractAlphaParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
    ...
    for(int i = 0; i < MLX90640_LINE_NUM; i++)
    {
        for(int j = 0; j < MLX90640_COLUMN_NUM; j ++)
        {
            ...
            alphaTemp[p] = alphaTemp[p] / (float)POW2(alphaScale);
    ...
    {
        temp = alphaTemp[i] * (float)POW2(alphaScale);        
...
static void ExtractKtaPixelParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
    ...
    for(int i = 0; i < MLX90640_LINE_NUM; i++)
    {
        for(int j = 0; j < MLX90640_COLUMN_NUM; j ++)
        {
            ...
            ktaTemp[p] = ktaTemp[p] / (float)POW2(ktaScale1);
    ...
    for(int i = 0; i < MLX90640_PIXEL_NUM; i++)
    {
        temp = ktaTemp[i] * (float)POW2(ktaScale1);
        if (temp < 0)
        {
...
static void ExtractKvPixelParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
    ...
    for(int i = 0; i < MLX90640_LINE_NUM; i++)
    {
        for(int j = 0; j < MLX90640_COLUMN_NUM; j ++)
        {
            ...
            split = 2*(p/32 - (p/64)*2) + p%2;
            kvTemp[p] = KvT[split];
            kvTemp[p] = kvTemp[p] / (float)POW2(kvScale);
        }
    }
    ...
    for(int i = 0; i < MLX90640_PIXEL_NUM; i++)
    {
        temp = kvTemp[i] * (float)POW2(kvScale);
        if (temp < 0)
        {
...
static void ExtractCPParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
    ...
    alphaSP[0] = alphaSP[0] /  (float)POW2(alphaScale);
    ...
    ktaScale1 = MLX90640_NIBBLE2(eeData[56]) + 8;    
    mlx90640->cpKta = cpKta / (float)POW2(ktaScale1);
    
    cpKv = (int8_t)MLX90640_MS_BYTE(eeData[59]);
    
    kvScale = MLX90640_NIBBLE3(eeData[56]);
    mlx90640->cpKv = cpKv / (float)POW2(kvScale);
